...
```
    private static X509Certificate[] verifySigner(
        ...
        // 不带长度前缀的 signatures["len-(签名算法ID-(len-signature))"]
        ...
        while (signatures.hasRemaining()) {
            signatureCount++;
            try {
                // 不带长度前缀的  signature 的结构：[签名算法ID-(len-signature)]
...
# V2 验签的主逻辑之二：验证摘要
接着注释7-4处执行的 ApkSigningBlockUtils.verifyIntegrity 函数，接着看代码： 
/frameworks/base/core/java/android/util/apk/ApkSigningBlockUtils.java  
代码段9:
```
...
由注释9-1和9-2处，可知：CHUNKED_SHA256 和 CHUNKED_SHA512 算法，执行 verifyIntegrityFor1MbChunkBasedAlgorithm 函数来验证；VERITY_CHUNKED_SHA256 算法，执行 verifyIntegrityForVerityBasedAlgorithm 函数来验证。  
我们先接着9-1，看代码：  
/frameworks/base/core/java/android/util/apk/ApkSigningBlockUtils.java  
代码段10:
```
