...
```
ForEachServiceInClass 会遍历上述的 service 链表，找到 classname 为 main 的 zygote 后，执行 StartIfNotDisabled 函数去启动 zygote 进程。 zygote 进程就是系统的第二个进程，由代码段3段=的分析可知，其对应的文件为 /frameworks/base/cmds/app_process/app_main.cpp。 
